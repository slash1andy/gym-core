...
class Manager {
	 ...
	 * @var string
	 */
	const COOKIE_NAME = 'gym_location';
	 ...
	 * @var string
	 */
	const USER_META_KEY = 'gym_location';
	 ...
	 * @var string
	 */
	const AJAX_ACTION = 'gym_set_location';
	...
	public function handle_ajax_set_location(): void {
		...
		if ( ! $this->set_location( $slug ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Invalid location.', 'gym-core' ) ),
				400
			);
			return;
		}
		...
		wp_send_json_success(
			array(
				'location' => $slug,
				'message'  => __( 'Location updated.', 'gym-core' ),
			)
		);
	}
