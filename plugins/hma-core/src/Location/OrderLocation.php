...
class OrderLocation {
	...
	public function render_order_location_filter(): void {
		...
		$selected = isset( $_GET['hma_location'] ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			? sanitize_key( wp_unslash( $_GET['hma_location'] ) ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			: '';

		$locations = array(
			''                 => __( 'All locations', 'hma-core' ),
			Taxonomy::ROCKFORD => __( 'Rockford', 'hma-core' ),
			Taxonomy::BELOIT   => __( 'Beloit', 'hma-core' ),
		);

		echo '<select name="hma_location" id="hma_location_filter">';
	...
	public function apply_legacy_order_filter( array $vars ): array {
		...
		if ( ! isset( $_GET['hma_location'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return $vars;
		}

		$location = sanitize_key( wp_unslash( $_GET['hma_location'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function apply_hpos_order_filter( array $clauses, $query, array $query_args ): array {
		...
		$clauses['join'] = ( $clauses['join'] ?? '' ) . $wpdb->prepare(
			" INNER JOIN {$wpdb->prefix}wc_orders_meta AS hma_loc ON hma_loc.order_id = {$wpdb->prefix}wc_orders.id AND hma_loc.meta_key = %s AND hma_loc.meta_value = %s",
			self::META_KEY,
			$location
		);
	...
	public function display_location_in_order_admin( \WC_Order $order ): void {
		...
		printf(
			'<p class="hma-order-location"><strong>%s</strong> %s</p>',
			esc_html__( 'Location:', 'hma-core' ),
			esc_html( $label )
		);
	}
}
