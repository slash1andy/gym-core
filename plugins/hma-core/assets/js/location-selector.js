 ...
 *
 * Data is passed from PHP via wp_localize_script under window.hmaLocation:
 *   ajaxUrl  {string} — wp-admin/admin-ajax.php URL
 *   nonce    {string} — hma_location_nonce value
...
( function () {
	...
	var selector = document.getElementById( 'hma-location-selector' );

	if ( ! selector ) {
		return;
	}

	var settings = window.hmaLocation || {};
	...
	function updateUI( slug ) {
		var buttons = selector.querySelectorAll( '.hma-location-selector__button' );

		buttons.forEach( function ( btn ) {
			var isActive = btn.dataset.location === slug;
			btn.classList.toggle( 'hma-location-selector__button--active', isActive );
			btn.setAttribute( 'aria-pressed', isActive ? 'true' : 'false' );
		} );

		selector.dataset.currentLocation = slug;
		selector.classList.remove( 'hma-location-selector--no-selection' );
	}
	...
	selector.addEventListener( 'click', function ( event ) {
		var button = event.target.closest( '.hma-location-selector__button' );
