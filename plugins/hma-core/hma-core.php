...
define( 'HMA_CORE_FILE', __FILE__ );
define( 'HMA_CORE_PATH', plugin_dir_path( __FILE__ ) );
define( 'HMA_CORE_URL', plugin_dir_url( __FILE__ ) );
define( 'HMA_CORE_BASENAME', plugin_basename( __FILE__ ) );

// Composer PSR-4 autoloader.
if ( file_exists( HMA_CORE_PATH . 'vendor/autoload.php' ) ) {
	require_once HMA_CORE_PATH . 'vendor/autoload.php';
}
...
register_activation_hook( HMA_CORE_FILE, array( 'HMA_Core\\Activator', 'activate' ) );
register_deactivation_hook( HMA_CORE_FILE, array( 'HMA_Core\\Deactivator', 'deactivate' ) );
	...
	static function (): void {
			...
			add_action(
				...
				static function (): void {
					echo '<div class="notice notice-error"><p>' .
						esc_html__( 'HMA Core requires WooCommerce to be installed and active.', 'hma-core' ) .
						'</p></div>';
				}
			);
			return;
		...
		if ( version_compare( PHP_VERSION, '8.0', '<' ) ) {
			add_action(
				...
				static function (): void {
					echo '<div class="notice notice-error"><p>' .
						esc_html__( 'HMA Core requires PHP 8.0 or higher.', 'hma-core' ) .
						'</p></div>';
				}
			);
			return;
